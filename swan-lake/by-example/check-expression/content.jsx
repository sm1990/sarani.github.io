...
<p><code>check E</code> is used with an expression <code>E</code> that might result in an <code>error</code> value.</p>

<p>If <code>E</code> results in an <code>error</code> value , then <code>check</code> makes the function return that <code>error</code> value</p>

<p>immediately.</p>

<p>Type of <code>check E</code> does not include <code>error</code>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=fc230a411f529a42fa6787225194be94&file=check_expression.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/check-expression", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run check_expression.bal`}</span>
