...
<p>A <code>stream</code> represents a sequence of values that are generated as needed. The end of a <code>stream</code> is indicated with a</p>

<p>termination value, which is <code>error</code> or <code>nil</code>. The type <code>stream&lt;T,E&gt;</code> is a <code>stream</code> where the members of the</p>

<p>sequence are type <code>T</code> and termination value is type <code>E</code>. <code>stream&lt;T&gt;</code> means <code>stream&lt;T,()&gt;</code>. The <code>stream</code> type</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=096c89ff373b44cf0eb1e58136607519&file=stream_type.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/stream-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run stream_type.bal`}</span>
<span>{`Retrieved even number: 2`}</span>
      ...
      <Link
        title="Join clause"
        href="/learn/by-example/join-clause"
      >
