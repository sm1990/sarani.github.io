...
<p>If stream terminates with <code>error</code>, result of <code>query expression</code> is an <code>error</code>. You cannot use <code>foreach</code></p>

<p>on <code>stream</code> type with termination type that allows <code>error</code>. Instead use <code>from</code> with <code>do</code> clause; the</p>

<p>result is a subtype of <code>error?</code>. Use <code>stream</code> keyword in front of <code>from</code> to create a <code>stream</code> which is</p>

<p>lazily evaluated. The failure of <code>check</code> within the <code>query</code> will cause the <code>stream</code> to produce an</p>

<p><code>error</code> termination value.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=ae4d9f0684be66c0139eae00748f1c28&file=querying_with_streams.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/querying-with-streams", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run querying_with_streams.bal`}</span>
...
<span>{`Non blank line count: 3`}</span>
      ...
      <Link
        title="Stream type"
        href="/learn/by-example/stream-type"
      >
      ...
      <Link
        title="Raw templates"
        href="/learn/by-example/raw-templates"
      >
