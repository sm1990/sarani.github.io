...
export default function ConvertingFromJsonToUserDefinedType() {
...
<p>Mutable structures have the <code>inherent</code> type that limits mutation.</p>

<p>Cast to <code>T</code> will work on mutable structure <code>s</code> only if the <code>inherent</code> type</p>

<p>of <code>s</code> is a subtype of <code>T</code>.</p>

<p>Casting immutable values will work but it does not do numeric conversions.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=d0449326852ac8e743fb59612603b994&file=converting_from_json_to_user_defined_type.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/converting-from-json-to-user-defined-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run converting_from_json_to_user_defined_type.bal`}</span>
<span>{`1.0`}</span>
<span>{`2.0`}</span>
</code></pre>
          </Col>
