...
export default function FtpListener() {
...
<p>The FTP listener is used to receive file/directory changes that occur in a remote</p>

<p>location using the FTP protocol. This sample includes receiving</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/ftp-listener", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run ftp_listener.bal`}</span>
<span>{``}</span>
...
<span>{`# latest polling will be printed for each of the polled events.`}</span>
      ...
      <Link
        title="Client"
        href="/learn/by-example/ftp-client"
      >
      ...
      <Link
        title="Client"
        href="/learn/by-example/sftp-client"
      >
