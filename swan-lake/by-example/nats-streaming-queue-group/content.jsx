...
export default function NatsStreamingQueueGroup() {
...
<p>The <code>nats</code> streaming library provides the built-in load balancing</p>

<p>feature called &quot;distributed queues&quot;. All subscribers with the</p>
...
<p>to receive the message. Although queue groups have multiple subscribers,</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/nats-streaming-queue-group", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`# `}<code>{`queue-group.bal`}</code>{` contains three services belonging to the same`}</span>
