...
<p><code>Query</code> can take advantage of <code>table</code> keys by using a <code>join clause</code>. Performs an <code>inner equijoin</code>.</p>

<p>The result is similar to using nested <code>from clause</code> and <code>where clause</code>. It is implemented as</p>

<p>a hash join: <code>table</code> keys allow you to avoid building a hash table. The type to join on must be <code>anydata</code>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=688d558e6a19884ea1de187d45f8d638&file=join_clause.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/join-clause", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run join_clause.bal`}</span>
