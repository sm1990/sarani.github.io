...
<p>Use <code>-&gt; W</code> or <code>&lt;- W</code> to send a message to or receive a message from worker <code>W</code> (use</p>

<p><code>function</code> to refer to the function's default worker).</p>

<p>The messages are copied using <code>clone()</code>. It implies immutable values are passed</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=060f5b4e50486781e071fb68ef49e285&file=inter_worker_message_passing.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/inter-worker-message-passing", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run inter_worker_message_passing.bal`}</span>
<span>{`3`}</span>
</code></pre>
          </Col>
