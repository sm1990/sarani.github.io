...
export default function LetClause() {
...
<p>Query expressions can have <code>let</code> clauses. They can be anywhere between <code>from</code> and <code>select</code></p>

<p>clauses. Multiple <code>where</code> clauses are also allowed. The semantics are similar to <code>XQuery FLWOR</code>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=6ff42800d4afc489df31f10a1d2a8490&file=let_clause.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/let-clause", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run let_clause.bal`}</span>
<span>{`["Melina Kodel","Tom Riddle"]`}</span>
      ...
      <Link
        title="Ordering"
        href="/learn/by-example/ordering"
      >
