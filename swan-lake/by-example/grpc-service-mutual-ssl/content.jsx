...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-service-mutual-ssl", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_service_mutual_ssl.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
