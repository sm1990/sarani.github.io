...
<p>The resources in Ballerina GraphQL services can have hierarchical resource</p>

<p>paths. When a hierarchical path is present, an <code>OBJECT</code> type is created for</p>

<p>each intermediate path segment with the same name. Every sub path under a</p>

<p>path segment will be added as a field of the created type.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying package, see the</p>

<p><a href="https://docs.central.ballerina.io/ballerina/graphql/latest/">GraphQL package</a>.&lt;br/&gt;&lt;br/&gt;</p>

<p>This example shows a GraphQL endpoint, which has a <code>profile</code> field of type <code>Person</code>.</p>

<p>A GraphQL client can query this service to retrieve specific fields or subfields of the <code>Person</code> object.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/graphql-hierarchical-resource-paths", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Send a query to the GraphQL endpoint using a cURL command.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run graphql_hierarchical_resource_paths.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
