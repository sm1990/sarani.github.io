...
<p>This works by creating a  RetryManager object <code>r</code>, before executing the transaction.</p>

<p>If the block fails with error <code>e</code>, it calls <code>r.shouldRetry(e)</code>.</p>

<p>If that returns <code>true</code>, then it executes the block again.</p>

<p><code>retry</code> has an optional type parameter giving class of <code>RetryManager</code> to create, and optional arguments to new <code>DefaultRetryManager</code> tries <code>n</code> times.</p>

<p><code>retry</code> can be used without <code>transaction</code>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=daa86b743409a42dfaeb9dbbdde496f1&file=retry_transaction_statement.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/retry-transaction-statement", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run retry_transaction_statement.bal`}</span>
...
<span>{`error: Stage2 failed`}</span>
      ...
      <Link
        title="Rollback"
        href="/learn/by-example/rollback"
      >
