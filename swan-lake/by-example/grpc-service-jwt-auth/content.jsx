...
export default function GrpcServiceJwtAuth() {
...
<p><code>Authorization</code> metadata against the provided configurations.&lt;br/&gt;</p>
...
<p>in the JWT using a custom claim attribute. That custom claim attribute</p>

<p>also can be configured as the <code>scopeKey</code>.&lt;br/&gt;</p>

<p>In the authorization phase, the scopes of the service/resource are compared</p>

<p>against the scope included in the JWT for at least one match between the two</p>

<p>sets.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://docs.central.ballerina.io/ballerina/jwt/latest/">JWT module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-service-jwt-auth", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_service_jwt_auth.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
