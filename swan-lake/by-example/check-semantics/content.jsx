...
<p><code>check</code> semantics is not to simply return on an <code>error</code> value.</p>

<p>When <code>check</code> gets an <code>error</code> value, it fails.</p>
...
<p><code>on fail</code> can catch the error.</p>

<p><code>fail</code> statement is like <code>check</code>, but it always fails.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=f5bae934b6333db5271b5f4a648306aa&file=check_semantics.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/check-semantics", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run check_semantics.bal`}</span>
<span>{`OK`}</span>
<span>{`OK`}</span>
