...
<p>An <code>xml</code> value belongs to <code>xml:Element</code> if it consists of just an element</p>

<p>item. Similarly for <code>xml:Comment</code> and <code>xml:ProcessingInstruction</code>.</p>

<p>An <code>xml</code> value belongs to <code>xml:Text</code> if it consists of a text item or is</p>

<p>empty.</p>

<p>An <code>xml</code> value belongs to the type <code>xml&lt;T&gt;</code> if each of its members belong</p>

<p>to <code>T</code>.</p>

<p>Functions in lang.xml use this to provide safe and convenient typing.</p>

<p>For example, <code>x.elements()</code> returns element items in <code>x</code> as type</p>

<p><code>xml&lt;xml:Element&gt;</code> and <code>e.getName()</code> and <code>e.setName()</code> are defined when</p>

<p><code>e</code> has type <code>xml:Element</code>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/xml-subtyping", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run xml_subtyping.bal`}</span>
