...
const codeSnippetData = [`import ballerina/cache;
import ballerina/io;
...
<p>The Ballerina Cache API provides an in-memory cache implementation by default with the</p>

<p><code>Least Recently Used</code> algorithm-based eviction policy.</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://docs.central.ballerina.io/ballerina/cache/latest/">Cache module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=95dce7220adb3c637dd7bd167595db08&file=cache_invalidation.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/cache-invalidation", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run cache_invalidation.bal`}</span>
<span>{`The existing keys in the cache: ["key1","key2","key3"]`}</span>
<span>{`The existing keys in after invalidating a given key: ["key1","key3"]`}</span>
<span>{`The keys after invalidating all the keys: []`}</span>
</code></pre>
          </Col>
      ...
      <Link
        title="Cache basics"
        href="/learn/by-example/cache-basics"
      >
