...
const ref2 = createRef()
  ...
  return (
    <Container className="d-flex flex-column h-100">
      <h1>Counter-based metrics</h1>


<p>Ballerina supports Observability out of the box and Metrics is one of the three important aspects of</p>

<p>Observability. To observe Ballerina code, the <code>--observability-included</code> build time flag should be given along with the</p>

<p><code>Config.toml</code> file when starting the service. The <code>Config.toml</code> file contains the required runtime configurations related to observability.</p>

<p>You can define and use metrics to measure your own logic. A counter is one type of the metrics that is</p>

<p>supported by default in Ballerina, and it is a cumulative metric that represents a single monotonically-increasing</p>

<p>counter whose value can only increase or be reset to zero.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information about configs and observing applications, see <a href="https://ballerina.io/learn/observing-ballerina-code/">Observing Ballerina Code</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/counter-metrics", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`// Invoke the service three times using the cURL command below.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`# To start the service, navigate to the directory that contains the`}</span>
<span>{`# `}<code>{`.bal`}</code>{` file and execute the `}<code>{`bal run`}</code>{` command below with the `}<code>{`--observability-included`}</code>{` build time flag and the `}<code>{`Config.toml`}</code>{` runtime configuration file.`}</span>
...
<span>{`Local Counter = 21`}</span>
<span>{`Registered Counter = 10`}</span>
<span>{`------------------------------------------`}</span>
<span>{`------------------------------------------`}</span>
<span>{`Global Counter = 2`}</span>
<span>{`Local Counter = 21`}</span>
<span>{`Registered Counter = 20`}</span>
<span>{`------------------------------------------`}</span>
<span>{`------------------------------------------`}</span>
<span>{`Global Counter = 3`}</span>
<span>{`Local Counter = 21`}</span>
<span>{`Registered Counter = 30`}</span>
<span>{`------------------------------------------`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
