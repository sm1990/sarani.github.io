...
export default function GrpcClientOauth2ClientCredentialsGrantType() {
...
<p>A client, which is secured with OAuth2 client credentials grant type</p>

<p>can be used to connect to a secured service.&lt;br/&gt;</p>

<p>The client metadata is enriched with the <code>Authorization: Bearer &lt;token&gt;</code></p>

<p>header by passing the <code>grpc:OAuth2ClientCredentialsGrantConfig</code> for the</p>

<p><code>auth</code> configuration of the client.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-client-oauth2-client-credentials-grant-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create a new Protocol Buffers definition file named `}<code>{`grpc_client.proto`}</code>{` and add the service definition to it.`}</span>
...
<span>{``}</span>
<span>{`# Once you run the command, `}<code>{`grpc_client_pb.bal`}</code>{` file is generated inside stubs directory.`}</span>
<span>{``}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_oauth2_client_credentials_grant_type.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
...
<span>{`Hello, World!`}</span>
      ...
      <Link
        ...
        href="/learn/by-example/grpc-client-oauth2-password-grant-type"
      >
