...
<p>Ballerina supports Observability out of the box and Metrics is one of the three important aspects of the</p>

<p>Observability. To observe Ballerina code, the build time flag <code>--observability-included</code> should be given along with the</p>
...
<p>The developers can define and use metrics to measure their own logic. A gauge is one type of the metrics that is</p>

<p>supported by default in Ballerina, and it represents a single numerical value that can arbitrarily go up and down,</p>

<p>and also based on the statistics configurations provided to the Gauge, it can also report the statistics such as max,</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/gauge-metrics", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`// Invoke the service three times using the cURL commands below.`}</span>
...
<span>{`------------------------------------------`}</span>
      ...
      <Link
        title="Kubernetes"
        href="/learn/by-example/c2c-deployment"
      >
