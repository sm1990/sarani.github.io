...
<p>The HTTP retry client tries sending over the same request to the backend service when there is a network level failure.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-retry", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# If the request that was sent to the `}<code>{`retry`}</code>{` resource fails due to an error, the client tries sending the request again.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run http_retry.bal.bal`}</span>
...
<span>{`time = 2021-01-21 19:00:48,404 level = INFO  module = "" message = "Request received from the client to healthy service."`}</span>
      ...
      <Link
        title="Failover"
        href="/learn/by-example/http-failover"
      >
      ...
      <Link
        title="Timeout"
        href="/learn/by-example/http-timeout"
      >
