...
<p>The <code>regex</code> library provides functionalities for searching, splitting, and replacing</p>

<p>the set of characters of the string by using the <code>regular expression</code>.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://docs.central.ballerina.io/ballerina/regex/latest/">Regex module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=7d56bb8ab0ed168899346b8fd22368be&file=regular_expressions.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/regular-expressions", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run regex_expressions.bal`}</span>
<span>{`Is the given string matched with the original string: true`}</span>
<span>{`Replaced string: Ballerina_is_great`}</span>
<span>{`String after replacing first Match: Replace thisTextThis`}</span>
<span>{`No of names: 4`}</span>
</code></pre>
          </Col>
