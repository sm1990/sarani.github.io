...
export default function GrpcClientOauth2JwtBearerGrantType() {
...
<p>A client, which is secured with an OAuth2 JWT bearer grant type can be</p>
...
<p>header by passing the <code>grpc:OAuth2JwtBearerGrantConfig</code> to the <code>auth</code></p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-client-oauth2-jwt-bearer-grant-type", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_oauth2_jwt_bearer_grant_type.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
