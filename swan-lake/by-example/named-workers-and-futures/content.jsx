...
<p>A reference to a named worker can be implicitly converted into a <code>future</code>.</p>

<p><code>start</code> is sugar for calling a function with a named worker and returning</p>

<p>the named worker as a <code>future</code>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=1577d8a25d219392882c9b9a1e49f533&file=named_workers_and_futures.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/named-workers-and-futures", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run named_workers_and_futures.bal`}</span>
<span>{`42`}</span>
<span>{`100`}</span>
</code></pre>
          </Col>
      ...
      <Link
        title="Multiple wait"
        href="/learn/by-example/multiple-wait"
      >
