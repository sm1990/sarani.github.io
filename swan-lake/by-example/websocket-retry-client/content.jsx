...
<p>If the WebSocket client lost the connection due to some transient failure, it automatically tries to</p>

<p>reconnect to the given backend. If the maximum reconnect attempt is reached it gives up on the connection. &lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://lib.ballerina.io/ballerina/websocket/latest/">WebSocket module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/websocket-retry-client", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# As a prerequisite, start a sample WebSocket service, which sends a message to the client upon upgrading to a WebSocket connection.`}</span>
<span>{`# If you are using a Ballerina WebSocket server, you can send a message to the client in the `}<code>{`onOpen`}</code>{` resource.`}</span>
