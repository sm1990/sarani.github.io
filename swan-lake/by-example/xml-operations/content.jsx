...
<p><code>x.id</code> accesses required attribute named <code>id</code>:</p>

<p>result is error if there is no such attribute</p>

<p>or if x is not a singleton.</p>

<p><code>x?.id</code> accesses optional attribute named <code>id</code>:</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=6c9425d387df042f5c36dc92716d2332&file=xml_operations.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/xml-operations", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run xml_operations.bal`}</span>
