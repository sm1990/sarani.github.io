...
<p>A gRPC service/resource can be secured with Basic Auth and by enforcing</p>

<p>authorization optionally. Then, it validates the Basic Auth token sent in</p>

<p>the <code>Authorization</code> metadata against the provided configurations. This reads</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-service-basic-auth-ldap-user-store", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_service_basic_auth_ldap_user_store.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
