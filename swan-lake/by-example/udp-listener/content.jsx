...
export default function UdpListener() {
...
<p>The UDP Listener is used to expose a UDP service over the UDP protocol.</p>

<p>The UDP <code>Client</code>/<code>ConnectClient</code> is used to send data to a remote UDP server.</p>

<p>This sample demonstrates the UDP socket listener.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/udp-listener", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run udp_listener.bal`}</span>
<span>{``}</span>
...
<span>{`Received by listener: Hello Ballerina echo`}</span>
      ...
      <Link
        title="Send emails"
        href="/learn/by-example/send-email"
      >
