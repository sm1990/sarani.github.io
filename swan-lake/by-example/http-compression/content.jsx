...
export default function HttpCompression() {
...
<p>The HTTP service can be configured to change the compression behaviour. By default, the server</p>

<p>compresses the response entity body with the scheme(gzip, deflate) that is specified in the Accept-Encoding request header. When</p>

<p>the particular header is not present or the header value is &quot;identity&quot;, the server does not perform any compression. Compression</p>

<p>is disabled when the option is set to <code>COMPRESSION_NEVER</code> and always enabled when the option is set to <code>COMPRESSION_ALWAYS</code>&lt;br/&gt;&lt;br/&gt;</p>

<p>In the same way <code>http:Client</code> can be configured as well. For more information on the underlying module,</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-compression", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# To invoke the alwaysCompress Service to get the String payload, use the following curl command.`}</span>
<span>{`# Here the `}<code>{`Accept-Encoding`}</code>{` header is not specified.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run http_compression.bal`}</span>
...
<span>{`ballerina: started HTTP/WS listener 0.0.0.0:9090`}</span>
      ...
      <Link
        title="Streaming"
        href="/learn/by-example/http-streaming"
      >
      ...
      <Link
        title="Trace logs"
        href="/learn/by-example/http-trace-logs"
      >
