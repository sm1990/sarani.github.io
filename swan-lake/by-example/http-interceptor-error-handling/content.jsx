...
<p>Errors that occurred in the request-response pipeline can be intercepted and handled</p>

<p>by <code>ResponseErrorInterceptors</code>. In addition, a <code>RequestErrorInterceptor</code></p>

<p>can be used to handle the errors that occurred in the request interceptor execution</p>

<p>path. The <code>RequestErrorInterceptor</code> can send a response message according to the</p>

<p>error just like a <code>ResponseErrorInterceptor</code>. Moreover, it can modify the</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-interceptor-error-handling", "_blank");
      }}
