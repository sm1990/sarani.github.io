...
<p>Ballerina supports generating Docker and Kubernetes artifacts from code without any additional configuration.</p>

<p>This simplifies the experience of developing and deploying Ballerina code in the cloud.</p>

<p>Code to Cloud builds the containers and required artifacts by deriving the required values from the code.</p>

<p>If you want to override the default values taken by the compiler, you can use a <code>Cloud.toml</code> file. &lt;br/&gt;&lt;br/&gt;</p>

<p>For more information, see <a href="/learn/run-ballerina-programs-in-the-cloud/code-to-cloud-deployment/">Code to Cloud deployment</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/c2c-deployment", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Before you build the package, you need to override some of the default values taken by the compiler. To do this, create a filed named `}<code>{`Cloud.toml`}</code>{` `}</span>
...
<span>{``}<code>{`eval \$(minikube docker-env)`}</code>{``}</span>
<span>{``}</span>
<span>{`# Execute the `}<code>{`bal build`}</code>{` command to build the Ballerina package. Code to Cloud generates only one container per package.`}</span>
<span>{``}<code>{`bal build`}</code>{``}</span>
<span>{``}</span>
...
<span>{``}</span>
<span>{`# Verify if the Docker image is generated. (If you have used the `}<code>{`minikube docker-env`}</code>{`, skip this and go to the Kubernetes deployment directly.)`}</span>
<span>{``}<code>{`docker images`}</code>{``}</span>
<span>{``}</span>
...
<span>{``}<code>{`docker run -d -p 9090:9090 wso2inc/hello:v0.1.0`}</code>{``}</span>
<span>{`c04194eb0b4d0d78cbc8ca55e0527d381d8ab4a1a68f8ea5dd3770a0845d5fbb`}</span>
<span>{``}</span>
<span>{`# Access the service.`}</span>
<span>{``}<code>{`curl http://localhost:9090/helloWorld/sayHello`}</code>{``}</span>
<span>{`Hello, World from service helloWorld !`}</span>
<span>{``}</span>
<span>{`# Execute the Kubernetes service now.`}</span>
<span>{``}<code>{`kubectl apply -f /home/wso2/project/target/kubernetes/hello-0.1.0`}</code>{``}</span>
...
<span>{``}<code>{`kubectl expose deployment wso2-hello-0--deployment --type=NodePort --name=wso2-hello-0--svc-local`}</code>{``}</span>
<span>{`service/wso2-hello-0--svc-local exposed`}</span>
<span>{``}</span>
<span>{`# Get the IP and port of the Kubernetes service.`}</span>
<span>{``}<code>{`kubectl get svc`}</code>{``}</span>
...
<span>{``}</span>
<span>{`# If you are using Minikube, the IP address should be changed according to the output of the `}<code>{`minikube ip`}</code>{` command.`}</span>
<span>{``}<code>{`minikube ip`}</code>{``}</span>
<span>{`192.168.49.2`}</span>
<span>{``}</span>
<span>{`# Access the deployed service via CURL.`}</span>
<span>{``}<code>{`curl http://192.168.49.2:32437/helloWorld/sayHello`}</code>{``}</span>
<span>{`Hello, World from service helloWorld !`}</span>
</code></pre>
          </Col>
