...
<p>authorization optionally. Then, it validates the OAuth2 token sent in the</p>

<p><code>Authorization</code> metadata against the provided configurations. This calls the</p>
...
<p>in a service can be bound to one/more scope(s). The scope can be included</p>

<p>in the introspection response using a custom claim attribute. That custom</p>

<p>claim attribute also can be configured as the <code>scopeKey</code>.&lt;br/&gt;</p>

<p>In the authorization phase, the scopes of the service/resource are compared</p>

<p>against the scope included in the introspection response for at least one</p>

<p>match between the two sets.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://docs.central.ballerina.io/ballerina/oauth2/latest/">OAuth2 module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-service-oauth2", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_service_oauth2.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
...
<span>{`bal run service/target/bin/service.jar`}</span>
      ...
      <Link
        ...
        href="/learn/by-example/grpc-client-ssl-tls"
      >
