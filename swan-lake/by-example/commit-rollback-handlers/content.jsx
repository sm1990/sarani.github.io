...
<p>Often code needs to get executed depending on whether a transaction is committed.</p>

<p>Testing the result of the <code>commit</code> within the transaction statement works. However, it is inconvenient</p>

<p>from a modularity perspective, particularly, when you want to undo changes on <code>rollback</code>.</p>

<p>This seems much worse in a distributed transaction when the transaction statement is in another program.</p>

<p>Ballerina provides <code>commit</code>/<code>rollback</code> handlers, which are functions that get executed when the decision</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=2ab76018ac7e2b510835ddf19de54398&file=commit_rollback_handlers.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/commit-rollback-handlers", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run commit_rollback_handlers.bal`}</span>
...
<span>{`Email sent.`}</span>
      ...
      <Link
        ...
        href="/learn/by-example/lock-statement"
      >
