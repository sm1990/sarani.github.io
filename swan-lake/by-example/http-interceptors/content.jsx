...
<p>Interceptors are used to execute some common logic such as logging, header manipulation,</p>

<p>state publishing, etc. for all the inbound requests and outbound responses. A</p>

<p><code>RequestInterceptor</code> can be used to intercept the request and execute some custom</p>

<p>logic. <code>RequestInterceptors</code> have a resource method, which will be executed</p>

<p>before dispatching the request to the actual resource in the target service. In addition,</p>

<p>a <code>ResponseInterceptor</code> can be used to intercept the response. <code>ResponseInterceptors</code></p>

<p>have a remote method, which will be executed before dispatching the response to the client.</p>

<p>A collection of these interceptors can be configured as a pipeline at the listener level or service level.</p>

<p>For more information, see the <a href="https://docs.central.ballerina.io/ballerina/http/latest/">HTTP module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-interceptors", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`# Run the service.`}</span>
<span>{`bal run http_interceptors.bal`}</span>
      ...
      <Link
        title="Passthrough"
        href="/learn/by-example/http-passthrough"
      >
