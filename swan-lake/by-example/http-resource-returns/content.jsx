...
export default function HttpResourceReturns() {
...
<p>The resource method can return <code>anydata</code> type, an <code>http:Response</code> object, <code>StatusCodeResponse</code> records along with</p>

<p><code>error?</code>. Instead of using an <code>http:Caller</code>, the response can be sent similarly by returning from the method.</p>

<p>When returning <code>anydata</code>, the <code>@http:Payload</code> annotation can be used to specify the <code>Content-type</code> of the response</p>

<p>additionally. Otherwise, the default content type of the respective return value type will be added.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-resource-returns", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`Run the cURL command below to invoke bank/branch resource.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run http_resource_returns.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
