...
const codeSnippetData = [`// This is the service definition for the bidirectional streaming scenario.
syntax = "proto3";
...
<p>In a gRPC bidirectional streaming scenario, the gRPC service and the client</p>

<p>operate when each other sends a sequence of messages using a read-write stream.</p>

<p>In such scenarios, the two streams operate independently. Therefore, clients and servers can read and write in any order.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-bidirectional-streaming", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create new Protocol Buffers definition file `}<code>{`grpc_bidirectional_streaming.proto`}</code>{` and add service definition.`}</span>
<span>{`# Run the command below in the Ballerina tools distribution for stub generation.`}</span>
<span>{`bal grpc --input grpc_bidirectional_streaming.proto  --output stubs`}</span>
<span>{``}</span>
<span>{`# Once you run the command, `}<code>{`grpc_bidirectional_streaming_pb.bal`}</code>{` file is generated inside stubs directory.`}</span>
<span>{``}</span>
...
<span>{`# Copy the generated `}<code>{`grpc_bidirectional_streaming_pb.bal`}</code>{` stub file to the package.`}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_bidirectional_streaming.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_bidirectional_streaming_client.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
