...
<p>Ballerina supports encoding and decoding multipart content in HTTP responses along with the nested parts.</p>

<p>When you request multiparts from an HTTP inbound response, you get an array of the parts of the body (an array of</p>

<p>entities). If the received parts contain nested parts, you can loop through the parent parts and get the child parts.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-response-with-multiparts", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`#To encode the outbound response with multiparts.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`# In the directory, which contains the `}<code>{`.bal`}</code>{` file, create a directory named `}<code>{`files`}</code>{`,`}</span>
<span>{`# and add an XML file named `}<code>{`test.xml`}</code>{` in it.`}</span>
...
<span>{`</ballerinalang>"`}</span>
      ...
      <Link
        title="Chunking"
        href="/learn/by-example/http-disable-chunking"
      >
