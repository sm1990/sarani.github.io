...
const codeSnippetData = [`// Calls the \`intAdd\` function and returns the result.
...
export default function TesterinaMockingFunctions() {
...
const ref1 = createRef()
  ...
  return (
    <Container className="d-flex flex-column h-100">
      <h1>Function mocking</h1>


<p>Mock functions allow you to hide the real function implementation and engage your own definition when running tests.</p>

<p>This allows you to isolate your tests from the other modules and functions.&lt;br/&gt;&lt;br/&gt;</p>

<p>Function mocks can be stubbed with return values or with another user-defined function,</p>

<p>which has the same signature as the original function.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information, see <a href="https://ballerina.io/learn/testing-ballerina-code/testing-quick-start/">Testing Ballerina Code</a></p>

<p>and the <a href="https://docs.central.ballerina.io/ballerina/test/latest/">Test Module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/testerina-mocking-functions", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# To run this sample, create a Ballerina project named `}<code>{`bbe_mocking`}</code>{` and create a `}<code>{`tests`}</code>{` directory inside.`}</span>
<span>{`# Replace the content of the `}<code>{`main.bal`}</code>{` and add the `}<code>{`main_test.bal`}</code>{` to the `}<code>{`tests`}</code>{` directory.`}</span>
<span>{`# Execute the `}<code>{`bal test`}</code>{` command below.`}</span>
<span>{`# Function mocking is not supported for testing single `}<code>{`.bal`}</code>{` files.`}</span>
<span>{``}</span>
