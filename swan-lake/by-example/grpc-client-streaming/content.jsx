...
const codeSnippetData = [`// This is the service definition for the client streaming scenario.
syntax = "proto3";
...
export default function GrpcClientStreaming() {
...
<p>The gRPC Server Connector is used to expose gRPC services over HTTP/2.</p>

<p>In a gRPC client streaming scenario, the client writes a sequence of messages and sends them to the server.</p>

<p>Once the client has finished writing the messages, it waits for the server to read them and return a response.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-client-streaming", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create new Protocol Buffers definition file `}<code>{`grpc_client_streaming.proto`}</code>{` and add service definition.`}</span>
...
<span>{``}</span>
<span>{`# Once you run the command, `}<code>{`grpc_client_streaming_pb.bal`}</code>{` file is generated inside stubs directory.`}</span>
<span>{``}</span>
...
<span>{`# Copy the generated `}<code>{`grpc_client_streaming_pb.bal`}</code>{` stub file to the package.`}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_streaming.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_streaming_client.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
...
<span>{`bal run client/target/bin/client.jar`}</span>
      ...
      <Link
        ...
        href="/learn/by-example/grpc-bidirectional-streaming"
      >
