...
<p>Each resource function inside the service represents a resolver function for a field in the root Query type.&lt;br/&gt;&lt;br/&gt;</p>
...
<p>This example shows a GraphQL endpoint, which has a field <code>profile</code> of type <code>Person</code>.</p>

<p>A GraphQL client can query on this service to retrieve specific fields or subfields of the <code>Person</code> object.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/graphql-returning-record-values", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run graphql_returning_record_values.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
