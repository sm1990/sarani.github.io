...
export default function ConvertingToUserDefinedType() {
...
<p>The <code>cloneWithType()</code> langlib function in the lang.value</p>

<p>module can be used to convert a value to a user-defined type.</p>

<p>Result recursively uses specified type as inherent type of new value.</p>

<p>Automatically performs numeric conversions as necessary.</p>

<p>Every part of the value is cloned including immutable structural values.</p>

<p>Graph structure is not preserved.</p>

<p>Variant <code>fromJsonWithType()</code> also does reverse of conversions done by</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=32c92cb48dc5fb0c209a927ebc4d3b6a&file=converting_to_user_defined_type.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/converting-to-user-defined-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run converting_to_user_defined_type.bal`}</span>
<span>{`1.0`}</span>
<span>{`1.0`}</span>
</code></pre>
          </Col>
