...
const codeSnippetData = [`// Defines the \`Update\` record type.
type Update record {
    int updateIndex;
    int stockMnt;
};
...
<p>If there is a fail or panic in the execution of the block, then the transaction is rolled back.</p>

<p>Transaction statement can also contain a rollback statement.</p>

<p>Every possible exit from a transaction block must be either <code>commit</code>, <code>rollback</code>, fail exit (e.g., from <code>check</code>), or panic exit.</p>

<p>Rollback does not automatically restore Ballerina variables to values before the transaction.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=e6081e0a0f9dad4baa5085dda2431a21&file=rollback.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/rollback", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run rollback.bal`}</span>
