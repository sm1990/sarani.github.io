...
export default function HttpClientDataBinding() {
...
<p>Through client data binding, the response payload can be accessed directly. The payload type is inferred from the</p>

<p>contextually-expected type or from the <code>targetType</code> argument. An <code>anydata</code> type or <code>http:Response</code> is expected as</p>

<p>the return value type along with the error.</p>

<p>When the user expects client data binding to happen, the HTTP error responses (4XX, 5XX) will be categorized</p>

<p>as an <code>error</code> (<code>http:ClientRequestError</code>, <code>http:RemoteServerError</code>) of the client remote operation.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-client-data-binding", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# To invoke the `}<code>{`/call/all`}</code>{` resource, use the cURL command below.`}</span>
<span>{`curl "http://localhost:9090/call/all"`}</span>
<span>{`{"name":"Smith", "age":15}`}</span>
<span>{``}</span>
<span>{`# To invoke the `}<code>{`/call/5xx`}</code>{` resource, use the cURL command below.`}</span>
<span>{`curl "http://localhost:9090/call/5xx"`}</span>
<span>{`{"code":501, "payload":"data-binding-failed-with-501"}`}</span>
<span>{``}</span>
<span>{`# To invoke the `}<code>{`/call/4xx`}</code>{` resource, use the cURL command below.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run http_client_data_binding.bal`}</span>
