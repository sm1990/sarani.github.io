...
<p>You can configure resources of HTTP services to restrict the types of media they consume and produce.</p>

<p>This is done through the <code>consumes</code> and <code>produces</code> annotation attributes of the <code>ResourceConfig</code> annotation,</p>

<p>which is used with resources.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-restrict-by-media-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# To invoke the service, execute the following cURL request.`}</span>
...
<span>{``}</span>
<span>{`# To invoke the service using unsupported media type, execute the following cURL request. The content type of the`}</span>
<span>{`# request is not listed under the `}<code>{`consumes`}</code>{` resource configuration.`}</span>
...
<span>{`# in the Accept header is not listed under the `}<code>{`produces`}</code>{` resource configuration.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run restrict_by_media_type.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
