...
const codeSnippetData = [`// This is the service definition of the simple scenario.
syntax = "proto3";
...
export default function GrpcSimpleWithHeaders() {
...
<p>In a simple RPC call, a client sends a request to a remote service and waits for the response.</p>

<p>The headers can be passed using the context record that is generated for each Protobuf message.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://docs.central.ballerina.io/ballerina/grpc/latest/">GRPC module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-simple-with-headers", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create a new Protocol Buffers definition file named `}<code>{`grpc_simple_with_headers.proto`}</code>{` and add the service definition.`}</span>
<span>{`# Run the command below in the Ballerina tools distribution for stub generation.`}</span>
<span>{``}<code>{`bal grpc --input grpc_simple_with_headers.proto  --output stubs`}</code>{``}</span>
<span>{``}</span>
<span>{`# Once you run the command, the `}<code>{`grpc_simple_with_headers_pb.bal`}</code>{` file is generated inside the `}<code>{`stubs`}</code>{` directory.`}</span>
<span>{``}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`# Create a Ballerina package.`}</span>
<span>{`# Copy the generated `}<code>{`grpc_simple_with_headers_pb.bal`}</code>{` stub file to the package.`}</span>
<span>{`# For example, if you create a package named `}<code>{`service`}</code>{`, copy the stub file to the `}<code>{`service`}</code>{` package.`}</span>
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_simple_with_headers.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
<span>{`# Execute the command below to build the 'service' package.`}</span>
<span>{``}<code>{`bal build service`}</code>{``}</span>
<span>{``}</span>
<span>{`# Run the service using the command below.`}</span>
<span>{``}<code>{`bal run service/target/bin/service.jar`}</code>{``}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref3}>
      <code className="d-flex flex-column"><span>{`# Create a Ballerina package.`}</span>
<span>{`# Copy the generated `}<code>{`grpc_simple_with_headers_pb.bal`}</code>{` stub file to the package.`}</span>
<span>{`# For example, if you create a package named `}<code>{`client`}</code>{`, copy the stub file to the `}<code>{`client`}</code>{` package.`}</span>
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_simple_with_headers_client.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
<span>{`# Execute the command below to build the 'client' package.`}</span>
<span>{``}<code>{`bal build client`}</code>{``}</span>
<span>{``}</span>
<span>{`# Run the client using the command below.`}</span>
<span>{``}<code>{`bal run client/target/bin/client.jar`}</code>{``}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick3 ? (
      ...
      <Link
        title="Simple RPC"
        href="/learn/by-example/grpc-simple"
      >
      ...
      <Link
        ...
        href="/learn/by-example/grpc-server-streaming"
      >
