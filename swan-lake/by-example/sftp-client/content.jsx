...
const codeSnippetData = [`import ballerina/ftp;
import ballerina/io;
...
<p>The SFTP client is used to perform CRUD operation on remote</p>

<p>files/directories using the SFTP protocol. This sample includes getting and</p>

<p>putting file content with default configurations using the default port</p>

<p>number.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://docs.central.ballerina.io/ballerina/ftp/latest/">FTP module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/sftp-client", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run sftp_client.bal`}</span>
<span>{``}</span>
<span>{`# File content of the received file would get printed.`}</span>
<span>{`# The newly-added file will appear in the SFTP server.`}</span>
</code></pre>
          </Col>
      ...
      <Link
        title="Listener"
        href="/learn/by-example/ftp-listener"
      >
