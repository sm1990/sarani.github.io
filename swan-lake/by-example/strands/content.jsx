...
<p>By default, named workers are multitasked cooperatively, not preemptively.</p>

<p>Each named worker has a &quot;strand&quot; (logical thread of control) and execution</p>

<p>switches between strands only at specific &quot;yield&quot; points such as doing a wait</p>

<p>or when a library function invokes a system call that would block.</p>

<p>This avoids the need for users to lock variables that are accessed from</p>

<p>multiple named workers.</p>

<p>An annotation can be used to make a strand run on a separate thread.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=11e44c9f9d071b7a1ab22cba055d53a4&file=strands.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/strands", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run strands.bal`}</span>
