...
export default function HttpErrorHandling() {
...
<p>Error handling is an integral part of any network program. Errors can be returned</p>

<p>by many components such as interceptors, dispatcher, data-binder, security handlers, etc.</p>

<p>These errors are often handled by a default handler and sent back as error responses</p>

<p>with an entity-body. With the introduction of error interceptors, you can intercept</p>

<p>these errors and handle them as you wish. These error interceptors can be placed</p>

<p>anywhere in the interceptor pipeline. When there is an error, the execution jumps</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-error-handling", "_blank");
      }}
...
<span>{`bal run http_error_handling.bal`}</span>
      ...
      <Link
        title="Interceptors"
        href="/learn/by-example/http-interceptors"
      >
