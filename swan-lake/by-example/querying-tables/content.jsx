...
export default function QueryingTables() {
...
<p>Tables can be nicely combined with <code>query expressions</code> unlike <code>maps</code>. Basic type of the output of</p>

<p><code>query expression</code> is determined by the contextually expected type and the input type.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=bc710e3abb9d0d515b9d515cb24404f6&file=querying_tables.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/querying-tables", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run querying_tables.bal`}</span>
