...
export default function TesterinaGroupTests() {
...
<p>You can tag your test cases with a single group name or multiple group names (one or more).</p>

<p>This allows you to control the execution of selected tests.&lt;br/&gt;&lt;br/&gt;</p>

<p>In order to execute tests belonging to a selected test group, you can name the</p>

<p>test groups that are to be executed when you run the tests.</p>

<p>Likewise, you can exclude executing the selected test groups as well.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/testerina-group-tests", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# To run this sample, navigate to the directory that contains the`}</span>
<span>{`# `}<code>{`.bal`}</code>{` file, and execute the `}<code>{`ballerina test`}</code>{` command below.`}</span>
<span>{``}</span>
<span>{`# Run the tests belonging to the `}<code>{`g1`}</code>{` and `}<code>{`g2`}</code>{` groups`}</span>
...
<span>{`        0 skipped`}</span>
<span>{``}</span>
<span>{`# Run the tests belonging to the `}<code>{`g1`}</code>{` group`}</span>
...
<span>{`        0 skipped`}</span>
<span>{``}</span>
<span>{`# Run all tests other than the tests belonging to the `}<code>{`g2`}</code>{` group`}</span>
