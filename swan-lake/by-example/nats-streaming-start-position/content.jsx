...
<p>The <code>nats</code> streaming library provides the functionality of historical</p>
...
<p>see the <a href="https://docs.central.ballerina.io/ballerinax/stan/latest">STAN module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/nats-streaming-start-position", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run publisher.bal`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`# When you start the subscriber after publishing several messages,`}</span>
<span>{`# You'll notice that,`}</span>
<span>{`# 1. `}<code>{`receiveSinceTimeDelta`}</code>{` service receives the messages if`}</span>
<span>{`#     the messages were sent within a historical offset of 5 seconds`}</span>
<span>{`#     from the current server date/time`}</span>
<span>{`# 2. `}<code>{`receiveFromGivenIndex`}</code>{` service receives services messages`}</span>
<span>{`#     starting from the third message published.`}</span>
<span>{`# 3. `}<code>{`receiveFromLastReceived`}</code>{` service receives messages starting`}</span>
<span>{`#     from the last published message.`}</span>
<span>{`# 4. `}<code>{`receiveFromBeginning`}</code>{` service receives all messages ever`}</span>
<span>{`#     published`}</span>
<span>{`# 5. `}<code>{`receiveNewOnly`}</code>{` service receives only the messages, which are`}</span>
