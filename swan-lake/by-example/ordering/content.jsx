...
<p>Ordering works consistently with <code>&lt;</code>, <code>&lt;=</code>, <code>&gt;</code>, <code>&gt;=</code> operators. Some comparisons involving</p>

<p><code>()</code> and <code>float NaN</code> are considered <code>unordered</code>. <code>order by</code> clause allows <code>expressions</code> not just</p>

<p><code>field access</code>. A library module can enable Unicode-aware sorting by providing a</p>

<p><code>unicode:sortKey(str, locale)</code> function.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=28aa323617d23745f363ed7367dd8d79&file=ordering.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/ordering", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run ordering.bal`}</span>
...
<span>{`Rocky Irving`}</span>
      ...
      <Link
        title="Limit clause"
        href="/learn/by-example/limit-clause"
      >
