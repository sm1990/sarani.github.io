...
<p>You can use the gRPC client to connect or interact with a gRPC listener</p>

<p>secured with SSL/TLS.</p>

<p>Provide the <code>grpc:ClientSecureSocket</code> configurations to the client to</p>

<p>initiate an HTTPS connection over HTTP/2.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-client-ssl-tls", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_ssl_tls.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
