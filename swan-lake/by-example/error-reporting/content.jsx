...
<p>Ballerina does not have exceptions. Errors are reported by functions returning</p>

<p><code>error</code> values.</p>

<p><code>error</code> is its own basic type.</p>

<p>The return type of a function that may return an <code>error</code> value will be a union with <code>error</code>.</p>

<p>An <code>error</code> value includes a <code>string</code> message.</p>

<p>An <code>error</code> value includes the stack trace from the point at which the error is constructed (i.e., <code>error(msg)</code> is called).</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=302682d6278101d41bb8c4d47b121e8e&file=error_reporting.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/error-reporting", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run error_reporting.bal`}</span>
<span>{`123`}</span>
<span>{`error("not a digit")`}</span>
      ...
      <Link
        title="Anydata type"
        href="/learn/by-example/anydata-type"
      >
