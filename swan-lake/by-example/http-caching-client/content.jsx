...
<p>Users can configure caching using the <code>cache</code> field in the client configurations.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-caching-client", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Invoke the service.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`# The two services have to be run separately to observe the following output.`}</span>
...
<span>{``}</span>
<span>{`# The backend service responds with a `}<code>{`200 OK`}</code>{` and it contains `}<code>{`etag`}</code>{` and `}<code>{`cache-control`}</code>{` headers. This response can be cached and as such, the caching client caches it. As seen from the `}<code>{`max-age`}</code>{` directive of the 'cache-control header, this response is valid for 15 seconds`}</span>
...
<span>{`Accept: */*`}</span>
<span>{``}</span>
<span>{`# This time, the request is not served from the cache. The backend service is contacted. The `}<code>{`if-none-match`}</code>{` header sends the entity tag of the now stale response, so that the backend service may determine whether this response is still valid.`}</span>
...
<span>{``}</span>
<span>{`# The response has not changed. Therefore the backend services respond with a `}<code>{`304 Not Modified`}</code>{` response. Based on this, the proxy will refresh the response, so that it can continue serving the cached response.`}</span>
...
<span>{`connection: keep-alive`}</span>
<span>{``}</span>
<span>{`# The service responds with a `}<code>{`200 OK`}</code>{` with the relevant caching headers set.`}</span>
...
<span>{`content-length: 0`}</span>
<span>{``}</span>
<span>{`# After checking the `}<code>{`if-none-match`}</code>{` header, the service determines that the response is still the same and that the proxy can keep reusing it.`}</span>
