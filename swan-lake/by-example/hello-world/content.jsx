...
<Row className="bbeCode px-2 py-0 rounded">
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=6ad5c5d62393547e093028efecfe59ca&file=hello_world.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.0.3/hello-world", "_blank");
      }}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
...
<Row className="bbeOutput p-2 rounded">
  <pre className="m-0">
    <code className="d-flex flex-column"><span>{`# To run this sample, place the source code in `}</span>
<span>{`# `}<code>{`hello_world.bal`}</code>{` file and use `}<code>{`bal run`}</code>{` command.`}</span>
