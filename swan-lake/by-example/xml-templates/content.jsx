...
<p><code>xml</code> values can be constructed using an XML template expression.</p>

<p>Phase 2 processing for <code>xml</code> template tag parses strings using</p>

<p>the XML 1.0 Recommendation's grammar for content (what XML allows</p>

<p>between a start-tag and an end-tag).</p>

<p>Interpolated expressions can be in content (<code>xml</code> or <code>string</code> values)</p>

<p>or in attribute values (<code>string</code> values).</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=9c770dc92466e079b138e7101b6aadc2&file=xml_templates.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/xml-templates", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run xml_templates.bal`}</span>
...
<span>{`<p><a href="https://ballerina.io">Ballerina</a> is an <em>exciting</em> new language!</p>`}</span>
      ...
      <Link
        ...
        href="/learn/by-example/xml-data-model"
      >
