...
export default function HttpFailover() {
...
<p>Ballerina users can configure multiple HTTP clients in a given failover group.</p>

<p>If one of the HTTP clients (dependencies) fails, Ballerina automatically fails over to another endpoint.</p>

<p>The following example depicts the <code>FailoverClient</code> behaviour with three target services. The first two targets</p>

<p>are configured to mimic failure backends.</p>

<p>After the first invocation the client resumes the failover from the last successful target. In this case it is</p>

<p>the third target and the client will get the immediate response for subsequent calls.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-failover", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# To invoke the Failover Service, use the cURL command below.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run http_failover.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
      ...
      <Link
        title="Retry"
        href="/learn/by-example/http-retry"
      >
