...
export default function JdbcQueryColumnMapping() {
...
<p>This BBE demonstrates how to use the JDBC  client for query operations with advanced mapping for column names.</p>

<p>Note that the relevant database driver JAR should be defined in the <code>Ballerina.toml</code> file as a dependency.</p>

<p>This sample is based on an H2 database and the H2 database driver JAR needs to be added to the <code>Ballerina.toml</code> file.</p>
...
<p>This sample is written using H2 2.0.6 and it is recommended to use an H2 JAR file of a version higher than 2.0.2.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/jdbc-query-column-mapping", "_blank");
      }}
...
<span>{`# Copy the example to the project and add the relevant database driver JAR details to the `}<code>{`Ballerina.toml`}</code>{` file.`}</span>
...
<span>{`Customer details: {"customerId":2,"firstName":"Dan","lastName":"Brown"}`}</span>
      ...
      <Link
        ...
        href="/learn/by-example/jdbc-query-row-operation"
      >
