...
const codeSnippetData = [`import ballerina/graphql;
import ballerina/io;
...
<p>GraphQL package provides a way to upload files through the GraphQL endpoints with GraphQL mutations. To define</p>

<p>an endpoint with the file upload capability, the <code>graphql:Upload</code> type can be used as the input parameter of</p>

<p>resolver functions. The <code>graphql:Upload</code> type can represent the details of the file that needs to be uploaded and</p>

<p>that can be used only with the remote functions. The value of <code>graphql:Upload</code> type is extracted from the HTTP</p>

<p>multipart request, which will be received by the GraphQL endpoints. This example shows how to implement a GraphQL endpoint that</p>
...
<p><a href="https://docs.central.ballerina.io/ballerina/graphql/latest/">GraphQL package</a>.</p>


<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  <Col sm={10}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/graphql-file-upload", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Send a HTTP multipart request to upload a single file via the GraphQL endpoint using a cURL command.`}</span>
<span>{`# The first part of the request is `}<code>{`operations`}</code>{` that includes a `}<code>{`JSON-encoded`}</code>{` map value.`}</span>
<span>{`# The `}<code>{`operations`}</code>{` map value is similar to a standard GraphQL POST request, in which all the variable values related to the file upload are `}<code>{`null`}</code>{`.`}</span>
<span>{`# `}<code>{`operations`}</code>{`: { "query": "mutation(\$file: Upload!) { singleFileUpload(file: \$file) }", "variables": {"file": null} }`}</span>
<span>{``}</span>
<span>{`# The second part of the request is a `}<code>{`map`}</code>{` field that includes a `}<code>{`JSON-encoded`}</code>{` map of files that occurred in the operations.`}</span>
<span>{`# The `}<code>{`key`}</code>{` is file field name and the `}<code>{`value`}</code>{` is an array of paths in which the files occurred in the `}<code>{`operations`}</code>{`.`}</span>
<span>{`# `}<code>{`map`}</code>{`: { “0”: ["variables.file"] }`}</span>
<span>{``}</span>
...
<span>{`# The variable value related to the files is an array of `}<code>{`null`}</code>{` values.`}</span>
<span>{`# `}<code>{`operations`}</code>{`: { "query": "mutation(\$file: [Upload!]!) { multipleFileUpload(files: \$file) }", "variables": { "file": [null, null] } }`}</span>
<span>{``}</span>
<span>{`# Same as the single file upload, the `}<code>{`map`}</code>{` value is a JSON-encoded map of paths in which files occurred in the `}<code>{`operations`}</code>{`.`}</span>
<span>{`# Since the `}<code>{`operations`}</code>{` has an array of `}<code>{`null`}</code>{` values, an array index is included in the path value.`}</span>
<span>{`#  E.g., In `}<code>{`file.0`}</code>{`, `}<code>{`0`}</code>{` is the array index.`}</span>
<span>{`# `}<code>{`map`}</code>{`: { "0": ["variables.file.0"], "1": ["variables.file.1"]}`}</span>
<span>{``}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run graphql_file_upload.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
