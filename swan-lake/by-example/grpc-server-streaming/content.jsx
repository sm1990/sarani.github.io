...
const codeSnippetData = [`// This is the service definition for the server streaming scenario.
syntax = "proto3";
...
export default function GrpcServerStreaming() {
...
<p>In a gRPC server streaming scenario, a client sends a request to the server and gets a stream to read the messages until all the messages are read.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-server-streaming", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create new Protocol Buffers definition file `}<code>{`grpc_server_streaming.proto`}</code>{` and add service definition.`}</span>
<span>{`# Run the command below in the Ballerina tools distribution for stub generation.`}</span>
<span>{`bal grpc --input grpc_server_streaming.proto  --output stubs`}</span>
<span>{``}</span>
<span>{`# Once you run the command, `}<code>{`grpc_server_streaming_pb.bal`}</code>{` file is generated inside stubs directory.`}</span>
<span>{``}</span>
...
<span>{`# Copy the generated `}<code>{`grpc_server_streaming_pb.bal`}</code>{` stub file to the package.`}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_server_streaming.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_server_streaming_client.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
