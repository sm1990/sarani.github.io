...
export default function GrpcClientOauth2PasswordGrantType() {
...
<p>A client, which is secured with OAuth2 password grant type can be used to</p>

<p>connect to a secured service.&lt;br/&gt;</p>

<p>The client metadata is enriched with the <code>Authorization: Bearer &lt;token&gt;</code></p>

<p>header by passing the <code>grpc:OAuth2PasswordGrantConfig</code> to the <code>auth</code></p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-client-oauth2-password-grant-type", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_oauth2_password_grant_type.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
