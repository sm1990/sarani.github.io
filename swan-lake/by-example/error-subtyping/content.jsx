...
export default function ErrorSubtyping() {
...
<p><code>distinct</code> creates a new subtype and can be used to define</p>

<p>subtypes of <code>error</code>.</p>

<p>The name of the distinct <code>error</code> type can be used with the error</p>

<p>constructor to create an <code>error</code> value of that type.</p>

<p>Works like a nominal type. The <code>is</code> operator can be used to</p>

<p>distinguish distinct subtypes.</p>

<p>Each occurrence of <code>distinct</code> has a unique identifier that is</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=e4b5b07a965c57a9ca9e52dd97dbd7ba&file=error_subtyping.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/error-subtyping", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run error_subtyping.bal`}</span>
<span>{`X`}</span>
</code></pre>
          </Col>
