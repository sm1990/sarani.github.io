...
export default function NamedWorkerReturnValues() {
...
<p>A <code>return</code> statement in a named worker terminates</p>

<p>the worker, not the function.</p>

<p>Similarly, when <code>check</code> is used and the expression evaluates</p>

<p>to an <code>error</code>, the <code>error</code> value is returned terminating the worker.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=a8c129511b65e2d03fc0038455a41de0&file=named_worker_return_values.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/named-worker-return-values", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run named_worker_return_values.bal`}</span>
...
<span>{`error: {ballerina/lang.int}NumberParsingError {"message":"'string' value '50m' cannot be converted to 'int'"}`}</span>
      ...
      <Link
        ...
        href="/learn/by-example/alternate-wait"
      >
