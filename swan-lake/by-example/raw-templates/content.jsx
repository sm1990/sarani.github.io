...
<p>A raw template is a backtick template without a tag. Exposes result of phase 1 without further processing.</p>

<p>Raw template is evaluated by evaluating each expression and creating an object containing.</p>

<p>&lt;ul&gt;</p>

<p>&lt;li&gt;an <code>array</code> of the <code>strings</code> separated by insertions&lt;/li&gt;</p>

<p>&lt;li&gt;an <code>array</code> of the results of expression evaluation and an <code>array</code> of <code>strings</code> separating&lt;/li&gt;</p>
...
<p>Note that the relevant database driver JAR should be defined in the <code>Ballerina.toml</code> file as a dependency.</p>

<p>This sample is based on an H2 database and the H2 database driver JAR need to be added to <code>Ballerina.toml</code> file.</p>

<p>For a sample configuration and more information on the underlying module, see the <a href="https://docs.central.ballerina.io/ballerinax/java.jdbc/latest/">JDBC module</a> .&lt;br&gt;&lt;br&gt;</p>

<p>This sample is written using H2 2.1.210 and it is recommended to use H2 JAR with versions higher than 2.1.210.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/raw-templates", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create a Ballerina project.`}</span>
<span>{`# Copy the example to the project and add relevant database driver jar details to the `}<code>{`Ballerina.toml`}</code>{` file.`}</span>
