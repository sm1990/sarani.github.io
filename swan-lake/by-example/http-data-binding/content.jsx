...
export default function HttpDataBinding() {
...
<p>HTTP service data binding helps to access the request payload through a resource signature parameter. The payload</p>

<p>parameter should be declared with the <code>@Payload</code> annotation and the  parameter type can be <code>anydata</code>.</p>

<p>Binding failures will be responded with 400[Bad Request] response&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-data-binding", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# To invoke the `}<code>{`student`}</code>{` resource, execute the below HTTP request.`}</span>
...
<span>{`{"Name":"John"}`}</span>
<span>{``}</span>
<span>{`# To invoke the `}<code>{`store`}</code>{` resource, execute the below HTTP request.`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run http_data_binding.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
