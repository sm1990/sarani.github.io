...
<p>(typically JSON) to user-defined type, using <code>cloneWithType</code>.</p>

<p>Return value that is subtype of <code>anydata</code> will be mapped from</p>

<p>user-defined type to protocol format, typically JSON, using <code>toJson</code>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/resource-method-typing", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`curl http://localhost:9090/calc/add -d "{\\"x\\": 1.0, \\"y\\": 2.0}"`}</span>
<span>{`3.0`}</span>
</code></pre>
          </Col>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run resource_method_typing.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
