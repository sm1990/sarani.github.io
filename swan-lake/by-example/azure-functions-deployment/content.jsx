...
const codeSnippetData = [`import ballerina/uuid;
...
<p>Ballerina functions can be deployed in Azure Functions by annotating a Ballerina function with Azure functions annotation&quot;. You can view the code examples below.</p>

<p>For more information, see the <a href="https://ballerina.io/learn/deployment/azure-functions/">Azure Deployment Guide</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/azure-functions-deployment", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Prerequisites: Azure CLI tools installation and configuration. Visit [Azure Deployment Guide](https://ballerina.io/learn/deployment/azure-functions/) for detailed steps.`}</span>
<span>{``}</span>
