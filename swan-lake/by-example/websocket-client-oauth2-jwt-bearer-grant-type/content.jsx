...
export default function WebsocketClientOauth2JwtBearerGrantType() {
...
<p>The client is enriched with the <code>Authorization: Bearer &lt;token&gt;</code> header by</p>

<p>passing the <code>websocket:OAuth2JwtBearerGrantConfig</code> to the <code>auth</code></p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/websocket-client-oauth2-jwt-bearer-grant-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# As a prerequisite, start a sample service secured with OAuth2.`}</span>
<span>{`# You may need to change the trusted certificate file path.`}</span>
<span>{`bal run websocket_client_oauth2_jwt_bearer_grant_type.bal`}</span>
