...
const codeSnippetData = [`// This is the service definition for the scenario.
syntax = "proto3";
...
<p>A gRPC service/resource can be secured with Basic Auth and optionally by</p>

<p>enforcing authorization. Then, it validates the Basic Auth token sent as the</p>

<p><code>Authorization</code> metadata against the provided configurations. This reads data</p>

<p>from a file, which has a TOML format. This stores the usernames, passwords</p>

<p>for authentication, and scopes for authorization.&lt;br/&gt;</p>

<p>Ballerina uses the concept of scopes for authorization. A resource declared</p>

<p>in a service can be bound to one/more scope(s).&lt;br/&gt;</p>

<p>In the authorization phase, the scopes of the service/resource are compared</p>

<p>against the scope included in the user store for at least one match between</p>

<p>the two sets.&lt;br/&gt;</p>

<p><code>Config.toml</code> has defined three users - alice, ldclakmal, and eve. Each user has a</p>

<p>password and optionally assigned scopes as an array.&lt;br/&gt;&lt;br/&gt;</p>

<p>For more information on the underlying module,</p>

<p>see the <a href="https://docs.central.ballerina.io/ballerina/auth/latest/">Auth module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-service-basic-auth-file-user-store", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create a new Protocol Buffers definition file named `}<code>{`grpc_service.proto`}</code>{` and add the service definition to it.`}</span>
...
<span>{``}</span>
<span>{`# Once you run the command, `}<code>{`grpc_service_pb.bal`}</code>{` file is generated inside stubs directory.`}</span>
<span>{``}</span>
...
<span>{`# Copy the generated `}<code>{`grpc_secured_pb.bal`}</code>{` stub file to the package.`}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_service_basic_auth_file_user_store.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
<span>{`# As a prerequisite, ensure that the `}<code>{`Config.toml`}</code>{` file is populated correctly`}</span>
