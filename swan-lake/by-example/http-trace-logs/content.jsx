...
<p>The HTTP trace logs can be used to monitor the HTTP traffic that goes in and out of Ballerina.</p>

<p>To enable trace logs, the log level has to be set to <code>TRACE</code> using the runtime argument:</p>

<p>&lt;br&gt; <code>-Cballerina.http.traceLogConsole=true</code>. &lt;br&gt;</p>

<p>The configurations can be set in the <code>Config.toml</code> file for advanced use cases such as specifying the file path</p>

<p>to save the trace logs and specifying the hostname and port of a socket service to publish the trace logs.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-trace-logs", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run http_trace_logs.bal -- -Cballerina.http.traceLogConsole=true`}</span>
<span>{`ballerina: HTTP trace log enabled`}</span>
<span>{``}</span>
<span>{`# In the logs, `}<code>{`http.downstream`}</code>{` refers to the HTTP traffic that flows between the client and Ballerina`}</span>
<span>{`# while `}<code>{`http.upstream`}</code>{` refers to the HTTP traffic that flows between Ballerina and the backend.`}</span>
<span>{``}</span>
...
<span>{`[2021-10-06 18:18:12,061] TRACE {http.tracelog.downstream} - [id: 0x91dfb8c7, correlatedSource: n/a, host:localhost/0:0:0:0:0:0:0:1:9090 - remote:/0:0:0:0:0:0:0:1:50367] UNREGISTERED`}</span>
      ...
      <Link
        title="Compression"
        href="/learn/by-example/http-compression"
      >
      ...
      <Link
        title="Access logs"
        href="/learn/by-example/http-access-logs"
      >
