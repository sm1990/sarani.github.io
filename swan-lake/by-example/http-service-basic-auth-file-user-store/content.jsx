...
export default function HttpServiceBasicAuthFileUserStore() {
...
<p>An HTTP service/resource can be secured with Basic Auth and optionally by</p>

<p>enforcing authorization. Then, it validates the Basic Auth token sent in the</p>

<p><code>Authorization</code> header against the provided configurations. This reads data</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/http-service-basic-auth-file-user-store", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# As a prerequisite, ensure that the `}<code>{`Config.toml`}</code>{` file is populated correctly`}</span>
