...
const codeSnippetData = [`import ballerina/log;
...
<p>The <code>log</code> library contains the application log handling functions.</p>


<p>For more information on the underlying module, see the <a href="https://docs.central.ballerina.io/ballerina/log/latest/">Log module</a>.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/logging-configuration", "_blank");
      }}
...
<p>To run this sample use the <code>bal run</code> command.</p>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run logging_configuration.bal`}</span>
...
<span>{`# As shown in the output, only the `}<code>{`INFO`}</code>{` and higher level logs are logged by default.`}</span>
<span>{``}</span>
<span>{`# The log level can be configured via a Ballerina configuration file.`}</span>
<span>{`# To set the global log level to `}<code>{`DEBUG`}</code>{`, place the entry given below in the `}<code>{`Config.toml`}</code>{` file and run the sample.`}</span>
<span>{`# \`\`\``}</span>
...
<span>{`# As shown in the output, now the `}<code>{`DEBUG`}</code>{` and higher level logs are logged.`}</span>
<span>{``}</span>
<span>{`# Each module can also be assigned its own log level. To assign a`}</span>
<span>{`# log level to a module, provide the following entry in the `}<code>{`Config.toml`}</code>{` file:`}</span>
<span>{`#`}</span>
<span>{`# \`\`\``}</span>
...
<span>{`# To set the output format to JSON, place the entry given below in the `}<code>{`Config.toml`}</code>{` file and run the sample.`}</span>
<span>{`# \`\`\``}</span>
<span>{`# [ballerina.log]`}</span>
...
<span>{`{"time":"2021-05-25T11:14:44.006+05:30", "level":"WARN", "module":"", "message":"warn log"}`}</span>
      ...
      <Link
        title="File paths"
        href="/learn/by-example/filepaths"
      >
