...
<p>Ballerina supports mutual SSL, which is a certificate-based authentication</p>

<p>process in which two parties (the client and server) authenticate each other by</p>

<p>verifying the digital certificates. It ensures that both parties are assured</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-client-mutual-ssl", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_mutual_ssl.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
