...
<p>The <code>graphql:Context</code> object can be used to pass meta information between the resolver functions. An init function</p>

<p>should be provided using the <code>graphql:ServiceConfig</code> parameter named <code>contextInit</code>. Inside the init function, the</p>

<p><code>graphql:Context</code> can be initialized. Values from the <code>http:RequestContext</code> and <code>http:Request</code> can be added as well as</p>

<p>other values. These values are stored as key-value pairs. The key is a <code>string</code> and the value can be any <code>readonly</code></p>

<p>value or an <code>isolated</code> object. If the init function is not provided, an empty context object will be created.</p>

<p>The context can be accessed by defining it as the first parameter of any resolver (resource/remote) function.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/graphql-context", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Send a query to the GraphQL endpoint using a cURL command. Set the `}<code>{`scope`}</code>{` header value to `}<code>{`admin`}</code>{`.`}</span>
...
<span>{``}</span>
<span>{` # Now, send a query with the `}<code>{`scope`}</code>{` header value set to `}<code>{`user`}</code>{`. This will return an error in the `}<code>{`salary`}</code>{` field.`}</span>
<span>{` # The query used: { profile { name salary } }`}</span>
...
<span>{``}</span>
<span>{` # Now, send a query with the `}<code>{`scope`}</code>{` header value set to `}<code>{`unknown`}</code>{`. This will return an error in the `}<code>{`profile`}</code>{` field.`}</span>
<span>{` # The query used: { profile { name salary } }`}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref2}>
      <code className="d-flex flex-column"><span>{`bal run graphql_context.bal`}</span>
</code></pre>
          </Col>
          <Col sm={2} className="d-flex align-items-start">
            {outputClick2 ? (
      ...
      <Link
        title="Mutations"
        href="/learn/by-example/graphql-mutations"
      >
