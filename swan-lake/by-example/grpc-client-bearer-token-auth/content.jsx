...
<p>A client, which is secured with Bearer token auth can be used to connect to</p>

<p>a secured service.&lt;br/&gt;</p>

<p>The client metadata is enriched with the <code>Authorization: Bearer &lt;token&gt;</code></p>

<p>header by passing the <code>grpc:BearerTokenConfig</code> for the <code>auth</code> configuration</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-client-bearer-token-auth", "_blank");
      }}
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_client_bearer_token_auth.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
