...
export default function GrpcSimple() {
...
<p>In a simple RPC call, a client sends a request to a remote service and waits for the response.&lt;br/&gt;&lt;br/&gt;</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/grpc-simple", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`# Create a new Protocol Buffers definition file named `}<code>{`grpc_simple.proto`}</code>{` and add the service definition.`}</span>
<span>{`# Run the command below from the Ballerina tools distribution for stub generation.`}</span>
<span>{``}<code>{`bal grpc --input grpc_simple.proto  --output stubs`}</code>{``}</span>
<span>{``}</span>
<span>{`# Once you run the command, the `}<code>{`grpc_simple_pb.bal`}</code>{` file is generated inside the `}<code>{`stubs`}</code>{` directory.`}</span>
<span>{``}</span>
...
<span>{`# Copy the generated `}<code>{`grpc_simple_pb.bal`}</code>{` stub file to the package.`}</span>
...
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_simple.bal`}</code>{` Ballerina file inside the `}<code>{`service`}</code>{` package and add the service implementation.`}</span>
<span>{``}</span>
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref3}>
      <code className="d-flex flex-column"><span>{`# Create a Ballerina package.`}</span>
<span>{`# Copy the generated `}<code>{`grpc_unary_blocking_pb.bal`}</code>{` stub file to the package.`}</span>
<span>{`# For example, if you create a package named `}<code>{`client`}</code>{`, copy the stub file to the `}<code>{`client`}</code>{` package.`}</span>
<span>{``}</span>
<span>{`# Create a new `}<code>{`grpc_unary_blocking_client.bal`}</code>{` Ballerina file inside the `}<code>{`client`}</code>{` package and add the client implementation.`}</span>
<span>{``}</span>
...
<span>{`bal run client/target/bin/client.jar`}</span>
      ...
      >
        ...
        <Col sm={6}>
      <Link
        title="Simple RPC with headers"
        href="/learn/by-example/grpc-simple-with-headers"
      >
