...
<p><code>any</code> means any value except an <code>error</code> value.</p>

<p>Equivalent to a union of all non-error basic types.</p>

<p>Use <code>any|error</code> for absolutely any value.</p>

<p>The <code>lang.value</code> lang library contains functions that apply to multiple basic types.</p>
...
<Row className="bbeCode mx-0 px-2 py-0 rounded" style={{ marginLeft: "0px" }}>
  ...
  <Col className="d-flex align-items-start pt-2" sm={2}>
    <button
        ...
        onClick={() => {
          window.open(
            "https://play.ballerina.io/?gist=d95132aab2f704a9b3de5c211cbef659&file=any_type.bal",
            "_blank"
          );
        }}
        target="_blank"
    ...
    <button
      ...
      onClick={() => {
        window.open("https://github.com/ballerina-platform/ballerina-distribution/tree/v2201.1.0/any-type", "_blank");
      }}
...
<Row className="bbeOutput mx-0 px-2 rounded">
  <Col className="my-2" sm={10}>
    <pre className="m-0" ref={ref1}>
      <code className="d-flex flex-column"><span>{`bal run any_type.bal`}</span>
<span>{`1`}</span>
<span>{`true`}</span>
<span>{`1.0`}</span>
</code></pre>
